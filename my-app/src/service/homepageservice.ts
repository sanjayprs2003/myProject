...
const HomeService = () => {
      ...
      if (userId && token) {
        const fetchExpense = async () => {
          try {
            ...
            const response = await apiService.sendRequest(HomeInterface.name, params, navigate) as HomeInterface.retrivel[];
      ...
      if (isConfirm) {
          try {
            const params: HomeInterface.param = { userId , categoryid : String(categoryid) };
              await apiService.sendRequest(HomeInterface.name_1, params, navigate) ;
    ...
    const handleUpdate = async () => {
      ...
      try {
        const params : HomeInterface.param = {userId : userId, 
        ...
        await apiService.sendRequest(HomeInterface.name_2, params, navigate);
