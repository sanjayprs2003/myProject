        ...
        if (userId && token) {
            const fetchExpenses = async () => {
                try {
                    ...
                    const response = await apiService.sendRequest(CategoryInterface.name , params, navigate) as string[];
    ...
    const handleSubmit = async (categoryType : string) => {
     if(userId && token) {
        try {
            ...
            const response = await apiService.sendRequest(CategoryInterface.name_1, params, navigate) as CategoryInterface.retrivel[];
