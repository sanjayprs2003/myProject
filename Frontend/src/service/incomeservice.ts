    ...
    if (userId && token) {
      const fetchIncome = async () => {
        try {
          ...
          const response = await apiService.sendRequest(IncomeInterface.view, params, navigate) as IncomeInterface.retrivel;
          setError("");
          setIncome(response);
        } catch (error) {
  ...
  const handleSubmit = async () => {
    if (userId && token && incomeValue !== undefined) {
      try {
        ...
        await apiService.sendRequest(IncomeInterface.addBy, params, navigate);
        setShowForm((prev) => !prev);
        setError("");
        setIncome((prevState) => ({
          ...prevState!,
